...
public extension UIButton {
    ...
    convenience init(swizzle: Bool) {
        ...
        let selector1 = #selector(UIButton.ga_sendAction(_:to:for:))
    ...
    @objc func ga_sendAction(_ action: Selector, to target: Any?, for event: UIEvent?) {

        if ga_eventUnavailable {return}
        
        if self.ga_eventAfterDelay > 0.0 {
            self.ga_eventUnavailable = true
            self.perform(#selector(setEventUnavailable(b:)), with: false, afterDelay: self.ga_eventAfterDelay)
        }
        
        self.ga_sendAction(action, to: target, for: event)
        
    }
    
    @objc func setEventUnavailable(b: Bool) {
        ga_eventUnavailable = b
    }
}
