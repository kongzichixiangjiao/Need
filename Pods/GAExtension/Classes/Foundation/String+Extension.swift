...
public extension String {
    ...
    // MARK:- 获取字符串大小
    func ga_getSize(fontSize: CGFloat) -> CGSize {
    ...
    // MARK:- 获取字符串大小
    func ga_getSize(font: UIFont) -> CGSize {
        let str = self as NSString
...
public extension String {
    func ga_positionOf(_ sub: String) -> Int {
        var pos = -1
    ...
    func ga_rangeOf(_ sub: String) -> Range<String.Index> {
...
public extension String {
    func ga_storyboard() -> UIStoryboard {
        return UIStoryboard(name: self, bundle: nil)
    }
    
    func ga_xib() -> UIViewController {
...
public extension String {
    ...
    func ga_transformToPinyinHead(lowercased: Bool = false) -> String {
        let pinyin = (self.ga_transformToPinyin(hasBlank: true) as AnyObject).capitalized // 字符串转换为首字母大写
        var headPinyinStr = ""
...
public extension String {
    func ga_checkEmpty(s: String) -> String {
        return self.isEmpty ? s : self
    }
}
