//
//  NSMutableAttributedString+public extension.swift
//  YueYe
//
...
public extension NSAttributedString {
    ...
    static func ga_paragraphStyle() -> NSMutableParagraphStyle {
    ...
    static func ga_contentStyle(_ text: String) -> NSAttributedString {
        let attributed = NSMutableAttributedString(string: text)
        let style = ga_paragraphStyle()
        attributed.addAttributes([NSAttributedString.Key.paragraphStyle : style], range: NSMakeRange(0, attributed.length))
        return attributed
    }

    // 字符串中有高亮字体 方法一：
    static func ga_highLightSubstring(allString: String, subString: String, color: UIColor) -> NSMutableAttributedString {
