...
protocol GAXibTabBarItemDelegate: class {
    func GAXibTabBarItemTap(title: String, tag: Int) -> Bool
}

@IBDesignable
class GAXibTabBarItem: UIView {
    
    weak var mDelegate: GAXibTabBarItemDelegate?
    ...
    @objc func tabbarItemTapAction(sender: UIButton) {
        guard let b = mDelegate?.GAXibTabBarItemTap(title: labelName, tag: mTag) else {
            return
        }
