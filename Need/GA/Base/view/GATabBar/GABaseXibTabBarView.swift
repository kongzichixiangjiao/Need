...
@objc protocol GABaseXibTabBarViewDelegate: NSObjectProtocol {
...
class GABaseXibTabBarView: UITabBar {
    
    weak var mDelegate: GABaseXibTabBarViewDelegate?
    
    var currentIndex: Int = 0
    
    var _specialView: UIView?
    
    @IBOutlet var buttons: [GAXibTabBarItem]!
    
    static func loadBaseXibTabBarView() -> GABaseXibTabBarView {
        return Bundle.main.loadNibNamed("GABaseXibTabBarView", owner: self, options: nil)?.last as! GABaseXibTabBarView
    }
...
extension GABaseXibTabBarView: UITabBarDelegate {
    func tabBar(_ tabBar: UITabBar, didSelect item: UITabBarItem) {
        print(tabBar, item)
    }
}

extension GABaseXibTabBarView: GAXibTabBarItemDelegate {
    func GAXibTabBarItemTap(title: String, tag: Int) -> Bool {
        if currentIndex == tag {
            mDelegate?.base_xibTabBarViewClickedCurrentItem?(index: currentIndex)
            return false 
        }
