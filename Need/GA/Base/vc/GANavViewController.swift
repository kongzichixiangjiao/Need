...
protocol GANavViewControllerProtocol {
    
}

extension GANavViewControllerProtocol where Self: GANavViewController {
    func nav_hideBackButton() {
...
class GANavViewController: GAViewController {
    ...
    public func b_setupNavigationView(height: CGFloat) {
        for constraint in b_navigationView.constraints {
            if constraint.firstAttribute == .height {
