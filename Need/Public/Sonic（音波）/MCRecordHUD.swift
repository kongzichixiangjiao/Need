...
class MCRecordHUD: UIView {
    ...
    convenience init(type: HUDType) {
        ...
        volume = MCVolumeView(frame: CGRect(x: VolumeViewLeftSpace, y: 0, width: VolumeViewWidth, height: VolumeViewHeight), type: type)
        addSubview(volume)
...
extension MCRecordHUD {
    ...
    private func setUpLabel() {
        ...
        let y: CGFloat = self.frame.size.height - h - 15
    ...
    func updateView(soundMeters: [Float]) {
        volume.updateView(soundMeters: soundMeters)
    }
}
